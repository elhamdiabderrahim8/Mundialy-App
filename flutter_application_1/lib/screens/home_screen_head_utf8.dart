...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      ...
      debugPrint('­ƒÆÑ Error loading data: $e');
    } finally {
  ...
  Future<void> _fetchLiveMode() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        // Mettre ├á jour l'Overlay si un match est ├®pingl├®
        if (liveMatches.isNotEmpty && await FlutterOverlayWindow.isActive()) {
           // On cherche si un de nos matchs live est celui de l'overlay (exemple simplifi├®)
  ...
  Future<void> _fetchUnifiedData() async {
    ...
    } catch (e) { debugPrint('­ƒÆÑ Error fetching unified data: $e'); }
  }
    ...
    return ListView(
      ...
      children: [
        ...
        if (_standings.isNotEmpty) ...[
          _SectionHeader(icon: Icons.leaderboard_rounded, title: 'GROUPES OFFICIELS', subtitle: 'Donn├®es en direct SofaScore', textColor: textColor),
          const SizedBox(height: 12),
          _GroupsAutoCarousel(groups: _standings, textColor: textColor),
          const SizedBox(height: 24),
        ],
        _SectionHeader(icon: Icons.rss_feed_rounded, title: 'ACTUALIT├ëS MONDIAL 2026', subtitle: 'Les derni├¿res infos SofaScore', textColor: textColor),
        const SizedBox(height: 12),
        _buildNewsHorizontalList(),
        const SizedBox(height: 24),
        _SectionHeader(icon: Icons.star_border_rounded, title: 'MATCHS ├Ç VENIR', subtitle: 'Calendrier officiel du tournoi', textColor: textColor),
        const SizedBox(height: 12),
  ...
  Widget _buildNewsHorizontalList() {
    if (_newsArticles.isEmpty) return const SizedBox(height: 280, child: _EmptyState(msg: 'Aucune actualit├® disponible.'));
  ...
  List<Widget> _buildUpcomingMatches(Color textColor) {
    if (_matches.isEmpty) return const [SizedBox(height: 150, child: _EmptyState(msg: 'Aucun match programm├®.'))];
  ...
  String? _normalizeRound(String raw) {
    ...
    if (v.contains('round of 16') || v.contains('8th finals') || v.contains('huiti├¿me')) return 'Round of 16';
    if (v.contains('quarter') || v.contains('quart')) return 'Quarter-finals';
    if (v.contains('semi') || v.contains('demi')) return 'Semi-finals';
    if (v.contains('third place') || v.contains('troisi├¿me')) return 'Third place';
  ...
  void _showFilterBottomSheet(int mode) {
    if (mode == 0) {
      ...
      showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: Colors.transparent, builder: (context) => _ListBottomSheet(title: mode == 1 ? 'CHOISIR ├ëQUIPE' : 'CHOISIR GROUPE', items: keys, selectedItem: selectedItem, isDark: isDark, onItemSelected: (idx) { _pageController.jumpToPage(idx); Navigator.pop(context); }));
    }
  }
  ...
  void _pinMatch(BuildContext context, LiveMatch match) async {
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Match ${match.homeTeam} ├®pingl├® sur l\'├®cran d\'accueil !'), backgroundColor: _kGold),
      );
    }
  }
}
...
class _MatchFilterBar extends StatelessWidget {
  ...
  static const _labels = ['Par Date', 'Par ├ëquipe', 'Par Groupe']; static const _icons = [Icons.calendar_month_rounded, Icons.groups_rounded, Icons.group_rounded];
  @override
  Widget build(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
